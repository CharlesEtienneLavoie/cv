...
get_stats <- function(data_scholar, author.name = author.name) {
  ...
  position <- sapply(authors, function(x) {
    # Fuzzy matching for author name
    position <- grep("lavoie", x)

    if (length(position) == 0) {
      return("Other")
    }
    # If multiple matches (unlikely for full name but possible for "Lavoie"), take the first one or refine logic
    position <- position[1]
